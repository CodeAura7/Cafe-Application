...
export async function runInTransaction(statements: readonly SqlStatement[]): Promise<void> {
  ...
  await new Promise<void>((resolve, reject) => {
    database.transaction(
      (transaction: {executeSql: (sql: string, params: unknown[]) => void}) => {
...
const initialProducts: ReadonlyArray<[string, number, string]> = [
  ['Time Pass', 50, 'FOOD'], ['Spicy Pav Patty', 70, 'FOOD'], ['Monaco Bites (8pcs)', 100, 'FOOD'],
  ['Cheesy Pocket (4pcs)', 120, 'FOOD'], ['Korean Bun', 120, 'FOOD'], ['Peri Peri Korean Bun', 140, 'FOOD'],
  ['Falafel Burger', 140, 'FOOD'], ['Garlic Toast (2 pcs)', 50, 'FOOD'], ['Malai Toast (2pcs)', 70, 'FOOD'],
  ['Hydrebadi Masala Toast', 80, 'FOOD'], ['Creamy Open Toast', 80, 'FOOD'], ['Paneer Panini', 90, 'FOOD'],
  ['Coleslaw Basket (6 pcs)', 130, 'FOOD'], ['Garlic Sticks with Cream Cheese', 100, 'FOOD'],
  ['Veggie Delight Cracker Pizza', 200, 'FOOD'], ['Tandoori Paneer Cracker Pizza', 200, 'FOOD'],
  ['Kalakhatta Tiktok', 80, 'BEVERAGES'], ['Bubble Gum Kiddie', 80, 'BEVERAGES'], ['Cranberry Castle', 80, 'BEVERAGES'],
  ['Blue Lagoon Sapphire', 80, 'BEVERAGES'], ['Green Apple Fizzy', 80, 'BEVERAGES'], ['Banarasi Lemon Tea (Tamtam)', 40, 'BEVERAGES'],
  ['Haldi Badam Milk', 50, 'BEVERAGES'], ['Coffee Kiss', 60, 'BEVERAGES'], ['Black Coffee Mojito', 80, 'BEVERAGES'],
  ['Cold Coffee Mocha', 100, 'BEVERAGES'], ['Chilled Dry Fruit Milkshake', 90, 'BEVERAGES'],
  ['Strawberry Boba Shake', 190, 'BEVERAGES'], ['Cranberry Boba Shake', 190, 'BEVERAGES'], ['Blueberry Boba Shake', 190, 'BEVERAGES'],
  ['Extra Amul Cheese', 30, 'OTHERS'],
];

async function seedInitialData(): Promise<void> {
  const timestamp = new Date().toISOString();
  const productStatements: SqlStatement[] = initialProducts.map(([name, price, category]) => ({
    ...
    params: [name, price, category, timestamp, timestamp, name],
  }));
  const tableStatements: SqlStatement[] = [1, 2, 3, 4, 5, 6].map(tableNumber => ({
  ...
  await runInTransaction([...productStatements, ...tableStatements]);
}
